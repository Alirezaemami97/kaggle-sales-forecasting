...
class QuantileTFT:
    ...
    def __init__(self, quantiles: list[float], cfg: TFTConfig, horizon: int, seed: int) -> None:
    ...
    def fit(self, targets: list[Any], fcovs: list[Any]) -> "QuantileTFT":
        import torch
        ...
        self.model.fit(series=targets, future_covariates=fcovs)
        return self
...
def backtest_tft_predictions(
    ...
    for fold in folds:
        ...
        if not targets:
            ...
            continue
        model = QuantileTFT(quantiles, cfg, horizon, seed).fit(targets, fcovs)
